...
# ─── DRF Serializers (inline to keep files minimal) ───────────────────────────
...
class ListingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'title', 'address', 'city', 'state', 'price', 'price_inr',
                  'bedrooms', 'bathrooms', 'sqft', 'photo_main', 'is_published',
                  'list_date', 'realtor_name']
...
def api_listings(request):
    ...
    for param, field, lookup in [
        ('city', 'city__icontains', None),
        ('state', 'state__iexact', None),
        ('bedrooms', 'bedrooms__gte', None),
        ('price', 'price__lte', None),
        ('q', 'title__icontains', None),
    ]:
...
def listings(request):
    ...
    return render(request, 'listings.html', {'listings': page})
...
def search(request):
    ...
    return render(request, 'search.html', {'listings': qs.order_by('-list_date'), 'values': q})
